...
async fn run() -> Result<()> {
    ...
    while j < args.len() {
        match args[j].as_str() {
            "--base-url" => {
                cfg.provider.base_url = args.get(j + 1).cloned().unwrap_or_default();
                j += 2;
            }
            "--model" => {
                cfg.provider.model = args.get(j + 1).cloned().unwrap_or_default();
                j += 2;
            }
            "--api-key" => {
                cfg.provider.api_key = args.get(j + 1).cloned().unwrap_or_default();
                j += 2;
            }
            _ => {
                j += 1;
...
async fn cmd_chat(cfg: &Config, tools: &ToolRegistry, store: &dyn SessionStore) -> Result<()> {
    let (session_id, resumed) = if let Some(id) = store.latest().await? {
        (id, true)
    } else {
...
async fn cmd_list(store: &dyn SessionStore) -> Result<()> {
    ...
    for id in &ids {
        if let Some(s) = store.load(id).await? {
            ...
            println!(
                "  {}  {}  {}  {}",
                &id[..8.min(id.len())].cyan(),
                tag.dimmed(),
                format!("{}/{}t", s.tokens_in, s.tokens_out).dimmed(),
                preview.dimmed()
            );
        }
    }
...
async fn repl_loop(
    ...
    let rl_cfg = RustyConfig::builder()
        .history_ignore_space(true)
        .edit_mode(EditMode::Vi)
        .build();
    let mut rl = Editor::<MoonHelper, DefaultHistory>::with_config(rl_cfg)?;
    rl.set_helper(Some(MoonHelper {
        completer: FilenameCompleter::default(),
    }));
    ...
    loop {
        ...
        match readline {
            Ok(line) => {
                ...
                let result = agent::run_agent(cfg, tools, store, &line, session_id, &mut |ev| {
                    render_event(ev, &flag)
                })
                .await;
                if let Err(e) = result {
                    eprintln!("\n{} {}", "✗".red(), e.to_string().red());
...
async fn handle_command(
    ...
    match parts[0] {
        ...
        "sessions" => {
            cmd_list(store).await?;
        }
        "status" => {
            println!("{}", "─ status ─".bold());
            println!(
                "  session  : {}",
                &current_id[..8.min(current_id.len())].cyan()
            );
...
async fn print_welcome(cfg: &Config, session_id: &str, resumed: bool, store: &dyn SessionStore) {
    ...
    println!(
        "  {}  {}",
        format!("max {}", cfg.agent.max_steps.unwrap_or(200)).dimmed(),
        "/help . Ctrl+D".dimmed()
    );
    if resumed {
        if let Ok(Some(s)) = store.load(session_id).await {
            println!(
                ...
                format!(
                    "restored step={} tok={}/{}",
                    s.step, s.tokens_in, s.tokens_out
                )
                .dimmed()
            );
        }
