...
const ACC: CColor = CColor::Rgb {
    r: 92,
    g: 156,
    b: 245,
};
const TXT: CColor = CColor::Rgb {
    r: 224,
    g: 224,
    b: 224,
};
const MUTED: CColor = CColor::Rgb {
    r: 96,
    g: 96,
    b: 96,
};
const OK: CColor = CColor::Rgb {
    r: 126,
    g: 207,
    b: 126,
};
const ERR: CColor = CColor::Rgb {
    r: 224,
    g: 80,
    b: 80,
};

fn clr(c: CColor) -> SetForegroundColor {
...
pub async fn run(
    ...
    let mut out = stdout();
    execute!(out, clr(ACC))?;
    write!(out, "vibe-agent")?;
    execute!(out, clr(TXT))?;
    writeln!(out, " · {}", cfg.provider.model)?;
    execute!(out, clr(MUTED))?;
    writeln!(
        out,
        "{}",
        t(
    ...
    let mut streamed_lines: usize = 0;
    ...
    loop {
        ...
        // ── prompt ──
        let prompt_label = if picking_model {
            t("pick model> ", "选模型> ")
        } else {
            "vibe-agent> ".to_string()
        };
        execute!(out, clr(ACC))?;
        write!(out, "{}", prompt_label)?;
        execute!(out, ResetColor)?;
        out.flush()?;
        ...
        match line.as_str() {
            ...
            "/model" | "/models" => {
                ...
                *MODEL_CACHE.lock().unwrap() = models.clone();
                ...
                execute!(out, clr(MUTED))?;
                writeln!(out, "  {}", t("type a number to pick", "输入数字选择"))?;
                execute!(out, ResetColor)?;
                out.flush()?;
                picking_model = true;
                continue;
            ...
            "/help" => {
                execute!(out, clr(ACC))?;
                writeln!(out, "  {}", t("commands:", "命令:"))?;
                execute!(out, clr(TXT))?;
                writeln!(
                    out,
                    "  /model        {}",
                    t("list available models from API", "从 API 获取可用模型列表")
                )?;
                execute!(out, clr(TXT))?;
                writeln!(
                    out,
                    "  <number>      {}",
                    t(
                        "pick a model by number (after /model)",
                        "用数字选模型(执行 /model 后)"
                    )
                )?;
                execute!(out, clr(TXT))?;
                writeln!(
                    out,
                    "  /status       {}",
                    t("show current model & config", "查看当前模型和配置")
                )?;
                execute!(out, clr(TXT))?;
                writeln!(out, "  /key <sk-..>  {}", t("set API key", "设置 API 密钥"))?;
                execute!(out, clr(TXT))?;
                writeln!(out, "  /clear        {}", t("clear screen", "清屏"))?;
                execute!(out, clr(TXT))?;
                writeln!(out, "  /exit         {}", t("quit", "退出"))?;
                execute!(out, clr(TXT))?;
                writeln!(
                    out,
                    "  /zh           {}",
                    t("switch to Chinese", "切换到中文")
                )?;
                execute!(out, clr(TXT))?;
                writeln!(
                    out,
                    "  /en           {}",
                    t("switch to English", "切换到英文")
                )?;
                execute!(out, clr(MUTED))?;
                writeln!(
                    out,
                ...
                writeln!(
                    out,
                    "  {}  ·  {}",
                    t("ctrl+c = exit", "ctrl+c = 退出"),
                    t("ctrl+d = exit", "ctrl+d = 退出")
                )?;
                execute!(out, ResetColor)?;
                out.flush()?;
                continue;
            ...
            s if picking_model && s.len() <= 2 && s.chars().all(|c| c.is_ascii_digit()) => {
                ...
                if idx > 0 && idx <= cache.len() {
                    ...
                    writeln!(
                        ...
                        t("model", "模型"),
                        cache[idx - 1],
                        t("restart to apply", "重启后生效")
                    )?;
                } else {
                    execute!(out, clr(ERR))?;
                    writeln!(out, "  invalid number, run /model first")?;
                }
                execute!(out, ResetColor)?;
                out.flush()?;
                continue;
            ...
            "/en" => {
                ...
                execute!(out, clr(OK))?;
                writeln!(out, "  switched to English")?;
                execute!(out, ResetColor)?;
                out.flush()?;
                continue;
            }
            "/status" => {
                execute!(out, clr(TXT))?;
                writeln!(out, "  model: {}", cfg.provider.model)?;
                execute!(out, ResetColor)?;
                out.flush()?;
                continue;
            }
            ...
            prompt => {
                ...
                agent_thread = Some(std::thread::spawn(move || {
                    ...
                    rt.block_on(async {
                        let _ =
                            crate::agent::run_agent(&c2, &t2, s2.as_ref(), &p, &sid, &mut |ev| {
                                tx2.send(ev).ok();
                            })
                            .await;
                    });
...
fn drain_events(
    ...
    text_buf: &mut String,
    streamed_lines: &mut usize,
    _thinking: &mut bool,
    _spin_idx: &mut usize,
    _spin_tick: &mut Instant,
    out: &mut impl Write,
) -> Result<()> {
        ...
        match &ev {
            ...
            AgentEvent::TextDone { .. } => {
                ...
                *streamed_lines += 1;
                ...
                // replace streamed text with markdown rendering if it fits on screen
                let (_, rows) = crossterm::terminal::size().unwrap_or((80, 24));
                if *streamed_lines > 0 && *streamed_lines < rows as usize / 2 {
                    // move cursor back up, clear, and re-print with markdown
                    execute!(
                        out,
                        cursor::MoveUp(*streamed_lines as u16),
                    ...
                    for line in &md_lines {
                        for span in &line.spans {
                            let c = span
                                .style
                                .fg
                                .unwrap_or(ratatui::style::Color::Rgb(224, 224, 224));
                            let (r, g, b) = match c {
...
fn show(ev: &AgentEvent) -> Result<()> {
    ...
    match ev {
        ...
        AgentEvent::Done {
            ...
            write!(o, "  done  ")?;
            execute!(o, clr(TXT))?;
            writeln!(o, "{} steps  {}/{} tokens", steps, tokens_in, tokens_out)?;
            execute!(o, ResetColor)?;
            o.flush()?;
        }
        AgentEvent::Error(e) => {
