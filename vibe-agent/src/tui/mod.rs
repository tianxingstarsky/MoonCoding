...
impl App {
    fn new(cfg: Arc<Config>) -> Self {
        ...
        chat.push(Line::from(vec![
            ...
            Span::styled("  ⏎ to send  ·  / for commands  ·  Esc to quit", Style::default().fg(Color::Rgb(60, 60, 60))),
        ]));
        ...
        let mut status = StatusBar::new("ready");
        status.set_right(&cfg.provider.model);
        ...
        tools: Arc<ToolRegistry>, store: Arc<dyn SessionStore>) -> Result<()>
    {
        ...
        loop {
            terminal.draw(|f| {
                ...
                self.chat.render(f, top[0], self.focus == Focus::Chat);
            ...
            if event::poll(std::time::Duration::from_millis(100))? {
                ...
                match key.code {
                    KeyCode::Tab => { self.focus = match self.focus { Focus::Input=>Focus::Chat, Focus::Chat=>Focus::Side, Focus::Side=>Focus::Input }; }
                    KeyCode::Char('c') if key.modifiers.contains(crossterm::event::KeyModifiers::CONTROL) => {
                        self.interrupt.store(true, Ordering::SeqCst); crate::agent::INTERRUPTED.store(true, Ordering::SeqCst);
                    ...
                    KeyCode::Enter => {
                        if self.input.command_mode {
                            ...
                            match cmd.trim() {
                                "/q"|"/quit"|"/exit" => break,
                                "/h"|"/help" => { self.chat.append_delta("Commands: /q quit /h help /s sessions /c clear /m model"); }
                                "/c"|"/clear" => { self.chat = ChatPanel::new(); }
                                "/s"|"/sessions" => { self.chat.append_delta("saved sessions:\n  (use /switch <id> to resume)"); }
                                "/switch" => { self.chat.append_delta("restart: vibe-agent resume <session-id>"); }
                                "/m"|"/model" => {
                                    self.chat.append_delta(&format!("model: {}\nbase_url: {}\napi_key: {}",
                                        self.cfg.provider.model, self.cfg.provider.base_url,
                    ...
                    KeyCode::Char(' ') => { let p=self.chat.scroll_pos(); let i=self.chat.line_count().saturating_sub(1+p); self.chat.toggle_line(i); }
    ...
    fn handle_event(&mut self, ev: AgentEvent) {
        match ev {
            ...
            AgentEvent::TextDone { .. } => {}
            AgentEvent::ToolCallStart { name, input, .. } => { self.chat.push_tool_start(&name, &input); self.status.set(&format!("running {}", name)); }
            AgentEvent::ToolCallResult { name, output, exit_code, .. } => { self.chat.push_tool_result(&name, exit_code, &output); }
            AgentEvent::Done { steps, tokens_in, tokens_out } => {
                self.thinking = false;
                self.side.set_entries(vec![("model".into(),self.cfg.provider.model.clone()),("tokens".into(),format!("{}/{}",tokens_in,tokens_out)),("steps".into(),steps.to_string())]);
