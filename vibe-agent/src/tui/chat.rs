...
impl ChatPanel {
    ...
    pub fn append_delta(&mut self, text: &str) {
        if let Some(ref mut cur) = self.current_assistant {
            ...
            if len > 0 {
                self.lines[len - 1].text = Line::from(Span::raw(content));
            }
        } else {
    ...
    pub fn push_tool_result(&mut self, name: &str, exit_code: i32, output: &str) {
        ...
        // update the tool start line with result info
        if let Some(line) = self.lines.iter_mut().rev()
            .find(|l| l.tool_call.as_ref().map(|tc| tc.name == name).unwrap_or(false))
        {
            line.tool_call.as_mut().unwrap().exit_code = exit_code;
            line.tool_call.as_mut().unwrap().full_output = output.to_string();
        }
        // also add a compact result line
        self.lines.push(ChatLine {
            text: Line::from(vec![
...
let row = line.text.clone();
            ...
            if line.expanded {
                if let Some(tc) = &line.tool_call {
                    for output_line in tc.full_output.lines() {
                        display_lines.push(Line::from(vec![
                            Span::styled("  | ".to_string(), Style::default().fg(Color::DarkGray)),
                            Span::styled(output_line.to_string(), Style::default().fg(Color::DarkGray)),
                        ]));
                    }
                }
            }
