...
impl Tool for GlobTool {
    ...
    async fn execute(&self, args: Value, ctx: &ToolContext) -> ToolResult {
        ...
        let re = match Regex::new(&re_str) {
            Ok(r) => r,
            Err(e) => return ToolResult { output: format!("glob regex err: {}", e), exit_code: 1, duration_ms: 0, truncated: false },
        };
        ...
        let mut walker = walkdir::WalkDir::new(&ctx.workspace)
            .max_depth(10)
            .follow_links(false)
            .into_iter()
        ...
        ToolResult { output: out, exit_code: 0, duration_ms: ms, truncated: matched >= max_matches }
    }
}
